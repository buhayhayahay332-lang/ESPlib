...
local ESP = {
    ...
    Drawing = {
        ...
        Boxes = {
            ...
            RotationSpeed = 300,
            Gradient      = true,
            GradientRGB1  = Color3.fromRGB(180, 0, 255),
            GradientRGB2  = Color3.fromRGB(255, 0, 150),
            GradientFill  = true,
            GradientFillRGB1 = Color3.fromRGB(180, 0, 255),
            GradientFillRGB2 = Color3.fromRGB(255, 0, 150),
            Filled = {
                Enabled      = false,
                Transparency = 0.75,
...
local function getMatchingViewmodel(character)
    ...
    for _, vm in pairs(viewmodels:GetChildren()) do
        ...
        if vmTorso then
            local d = (vmTorso.Position - root.Position).Magnitude
...
local function updateNameLabel(character, label)
    ...
    local centerX, topY
    local matchedVM = getMatchingViewmodel(character)
    if matchedVM then
...
Workspace.ChildRemoved:Connect(function(child)
    if ActiveNames[child] then removeNameLabel(child) end
end)
